...
class NUnitRunner
	...
	def initialize(paths)
		...
		@nunitExe = File.join('tools', 'nunit', "nunit-console#{(@compilePlatform.empty? ? '' : "-#{@compilePlatform}")}.exe").gsub('/','\\') + ' /nothread'
	end
...
class MSpecRunner
	...
	def initialize(paths)
		...
		@mspecExe = File.join('tools', 'mspec', "mspec.exe").gsub('/','\\')
	end
	...
	def executeTests(assemblies)
		...
		assemblies.each do |assem|
			file = File.expand_path("#{@sourceDir}/#{assem}/bin/#{@compileTarget}/#{assem}.dll")
                        sh "#{@mspecExe} #{file} --html ./#{@resultsDir }"
		end
	end
end
